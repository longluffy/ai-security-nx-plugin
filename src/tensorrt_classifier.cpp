...
void TensorRTClassifier::classifyDetections(const cv::Mat& frame, DetectionList& detections) {
    ...
    const int minRoiSize = 32; // Minimum ROI size for classification (performance optimization)
    
    for (auto& detection : detections) {
        try {
            // Convert NX bounding box to OpenCV Rect
            cv::Rect roi;
            roi.x = static_cast<int>(detection->boundingBox.x * frameSize.width);
            roi.y = static_cast<int>(detection->boundingBox.y * frameSize.height);
            roi.width = static_cast<int>(detection->boundingBox.width * frameSize.width);
            roi.height = static_cast<int>(detection->boundingBox.height * frameSize.height);
            
            // Skip classification for very small ROIs (performance optimization)
            ...
            roi.width = std::min(roi.width, frameSize.width - roi.x);
            roi.height = std::min(roi.height, frameSize.height - roi.y);
            ...
            if (roi.width > 0 && roi.height > 0) {
                ...
                // Classify the ROI
                ClassificationResult result = classifyROI(roiImage);
                
                if (result.success) {
                    detection->classLabel = result.classLabel;
                    // Optionally store classification confidence
                    // detection->classificationConfidence = result.confidence;
                } else {
                    detection->classLabel = "Unknown";
                }
            } else {
